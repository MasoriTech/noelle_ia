...
# Fallback: depois que o navegador descarta site conhecido, "abra hydra" deve ser app local.
APP_OPEN_FALLBACK_PREFIXES = ('abra ', 'abre ', 'abrir ', 'abri ', 'inicie ', 'iniciar ', 'execute ', 'executa ', 'rode ', 'rodar ')
AVATAR_PREFIXES = ('/avatar','/bridge','/godot')
DOWNLOAD_PREFIXES = ('/baixar','/download','/downloads')
NOELLE_KOBOLD_PREFIXES = ('/noelle','/kobold')
...
def route_message(text: str, default_model: str = 'auto') -> Route:
    ...
    if _any_cmd_prefix(n, NOELLE_KOBOLD_PREFIXES):
        return Route('local', payload='Use /noelle status, /kobold status ou python -m yoru_bridge embedded.')
    if _cmd(n, '/api'):
        return Route('local', payload='API HTTP removida. Use integração embutida STDIO com KoboldCpp/Yoru.')
    ...
    if looks_like_browser_command(raw):
        return Route('browser', command=raw)
    # Fallback App Intent: se o usuário disse "abra hydra" e não é site conhecido, abre app local.
    if any(n.startswith(p) for p in APP_OPEN_FALLBACK_PREFIXES):
        return Route('apps', command=raw)
