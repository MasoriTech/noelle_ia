...
SKILL_CATALOG: tuple[SkillItem, ...] = (
    ...
    SkillItem('Noelle Kobold Replace', 'companion', ('python -m yoru_bridge embedded', '/noelle status', '/kobold status'), 'substitui completamente o chat/Ollama do Noelle por Yoru+KoboldCpp via STDIO JSONL'),
...
def build_capabilities_text(config: Dict[str, Any] | None = None) -> str:
    ...
    lines.extend([
        ...
        "12. NoelleKoboldReplace: python -m yoru_bridge embedded; Yoru+KoboldCpp substituem o chat/Ollama do Electron sem API HTTP.",
        "13. Diagnóstico: /mega check, /perf, /check, /modelos, /cache.",
        "",
...
def build_skills_status(config: Dict[str, Any] | None = None) -> str:
    ...
    rows = [
        ...
        ('NoelleKoboldReplace', _config_bool(config, 'noelle_kobold_replace', 'enabled', True), 'python -m yoru_bridge embedded'),
...
def build_skills_examples(config: Dict[str, Any] | None = None) -> str:
    return '\n'.join([
        ...
        '- /noelle status -> mostra o estado da substituição Ollama -> Yoru/KoboldCpp.',
    ])
