...
def iniciar_noelle_kobold_replace():
    print('NoelleKoboldReplace: use este pack como yoru_chat/ no GitHub do Noelle.')
    print('Entrada embutida: python -m yoru_bridge embedded')
    print('Patch: node yoru_chat/noelle_kobold_replace/apply_noelle_kobold_replace_2026.cjs')
    print('Diagnóstico: node yoru_chat/noelle_kobold_replace/diagnostico_noelle_kobold_replace_2026.cjs')
    try:
        subprocess.run([PYEXE, str(PACK_ROOT/'scripts'/'test_embedded_stdio.py')], cwd=str(PACK_ROOT))
    except KeyboardInterrupt:
        print('\n[OK] Diagnóstico encerrado.')


def main():
    while True:
        print("""\n===============================================================
 Yoru Bridge Modern 1.8.44 - RepoOrganized - 2026
===============================================================
ORDEM RECOMENDADA:
...
[17] NoelleKoboldReplace - trocar Ollama por Kobold/Yoru
[0] Sair""")
        try:
            c=input('Escolha: ').strip()
        except KeyboardInterrupt:
        ...
        elif c=='17': iniciar_noelle_kobold_replace()
        elif c=='0': return
