...
function log(msg) {
  console.log("[controls-v41.1] " + msg);
}

function deactivateTag(html, fileName) {
  const escaped = fileName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  html = html.replace(new RegExp(`<script[^>]*${escaped}[^>]*><\\/script>`, "g"), `<!-- disabled ${fileName} by avatar_v41_1 -->`);
  html = html.replace(new RegExp(`<link[^>]*${escaped}[^>]*>`, "g"), `<!-- disabled ${fileName} by avatar_v41_1 -->`);
  return html;
}
...
const backup = controlsPath + ".bak_v41_1";
if (!fs.existsSync(backup)) {
  fs.copyFileSync(controlsPath, backup);
  log("backup criado: src/controls.html.bak_v41_1");
}

let html = fs.readFileSync(controlsPath, "utf8");

const blocked = [
  "model_manifest_v41_1.js",
  "avatar_page_owner_v41_1.js",
...
const manifestTag = '<script src="./renderer/pages/avatar/model_manifest_v41_1.js"></script>';
const ownerTag = '<script src="./renderer/pages/avatar/avatar_page_owner_v41_1.js"></script>';
...
log("manifest JS + owner v41.1 injetados");