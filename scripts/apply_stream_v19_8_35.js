...
function log(msg) {
  console.log("[stream-v19.8.35] " + msg);
}

function backup(file) {
  if (!fs.existsSync(file)) return;
  const bak = file + ".bak_stream_v19_8_35";
...
function disableTag(html, fileName) {
  ...
  return html.replace(
    ...
    `<!-- disabled ${fileName} by stream_v19_8_35 -->`
  );
}
...
function patchControlsHtml() {
  ...
  html = disableTag(html, "noelle_stream_tts_runtime_v19_8_35.js");

  const tag = '<script src="./renderer/modules/noelle_stream_tts_runtime_v19_8_35.js"></script>';
  ...
  log("TTS runtime v19.8.35 injetado em controls.html");
}
