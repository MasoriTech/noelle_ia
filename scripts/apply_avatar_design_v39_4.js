...
function log(message) {
  console.log('[avatar-v39.4] ' + message);
}
...
function disableOldAvatarRuntimes(html) {
  const blocked = [
    ...
    'avatar_design_v39_4.css',
    'avatar_loadfile_size_640'
  ];

  for (const name of blocked) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    html = html.replace(new RegExp(`<script[^>]*${escaped}[^>]*><\\/script>`, 'g'), `<!-- disabled ${name} by avatar_design_v39_4 -->`);
    html = html.replace(new RegExp(`<link[^>]*${escaped}[^>]*>`, 'g'), `<!-- disabled ${name} by avatar_design_v39_4 -->`);
  }
  return html;
}
...
function patchControls() {
  ...
  html = disableOldAvatarRuntimes(html);

  const cssTag = '<link rel="stylesheet" href="./renderer/pages/avatar/avatar_design_v39_4.css">';
  const jsTag = '<script src="./renderer/pages/avatar/avatar_design_owner_v39_4.js"></script>';

  if (!html.includes('avatar_design_v39_4.css')) {
    html = html.includes('</head>') ? html.replace('</head>', cssTag + '\n</head>') : cssTag + '\n' + html;
    log('CSS v39.4 injetado');
  }

  if (!html.includes('avatar_design_owner_v39_4.js')) {
    html = html.replace('</body>', jsTag + '\n</body>');
    log('owner v39.4 injetado');
  }
...
function checkRequiredFiles() {
  ...
  for (const file of required) {
    log((exists(file) ? '[OK] ' : '[AVISO] faltando ') + file);
  }
}
