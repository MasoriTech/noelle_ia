...
function extractFunction(code, name) {
  ...
  if (idx < 0) return "";
  return code.slice(idx, idx + 900);
}

function main() {
  log("================================================================");
  log(" Diagnóstico V19.8.26 - Main performance finish");
  log("================================================================");
  ...
  if (!exists("main.js")) {
    ...
    else err("main.js não importa writeJsonAtomic");

    const writeJson = extractFunction(mainCode, "writeJson");
    if (writeJson.includes("writeJsonAtomic(file, value)")) ok("writeJson usa escrita atômica");
    else {
      err("writeJson ainda não usa escrita atômica");
      console.log("----- writeJson -----");
      console.log(writeJson || "(não encontrado)");
    }

    if (/function\s+writeJson[\s\S]{0,400}fs\.writeFileSync/.test(mainCode)) err("writeJson ainda usa fs.writeFileSync diretamente");
    ...
    if (ollama.includes("agent: OLLAMA_HTTP_AGENT")) ok("ollamaRequest usa OLLAMA_HTTP_AGENT");
      ...
      err("ollamaRequest não usa OLLAMA_HTTP_AGENT");
      console.log("----- ollamaRequest trecho -----");
      console.log(ollama || "(não encontrado)");
    }
    ...
    else warn("saveState sem atualização de cache V19.8.26");
  }
  ...
  if (exists("package.json")) {
    try {
      ...
      if (pkg.scripts && pkg.scripts["repair:v19.8.26-main-perf-finish"]) ok("package.json contém repair V19.8.26");
      else warn("package.json sem repair V19.8.26");
    } catch (e) {
      err("package.json inválido: " + e.message);
    }
  }

  if (process.exitCode) err("Diagnóstico V19.8.26 encontrou problemas.");
  else ok("Diagnóstico V19.8.26 aprovado.");
}
