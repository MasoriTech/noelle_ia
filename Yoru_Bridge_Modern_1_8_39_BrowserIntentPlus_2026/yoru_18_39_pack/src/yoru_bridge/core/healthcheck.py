...
def run_pack_healthcheck(config: Dict[str, Any] | None = None) -> str:
    ...
    cfg = config or {}
    lines: List[str] = []
    lines.append('=== Yoru Mega Pack Check 1.8.39 / DownloadCenter ===')
    ...
    lines.append('Comandos úteis: /baixar status, /baixar tudo, /contratos exemplos, /scope status, /tts status, /avatar eventos, /pc status, /modelo api.')
    return '\n'.join(lines)
