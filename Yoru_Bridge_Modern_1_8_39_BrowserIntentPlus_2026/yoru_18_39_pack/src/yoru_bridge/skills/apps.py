...
class AppInventorySkill:
    ...
    def handle(self, text: str) -> Optional[str]:
        ...
        if any(n.startswith(p) for p in _APP_CMD_PREFIXES):
    ...
    def _extract_natural_launch_query(self, raw: str) -> str:
        n = normalize(raw)
        for p in _APP_CMD_PREFIXES:
            if n.startswith(p):
                # Use o raw original, mas corte pelo tamanho aproximado do prefixo normalizado.
                # Fallback simples: remove verbos/artigos comuns por regex.
                q = re.sub(r'^(abra|abre|abrir|execute|executa|rodar|rode|inicie|iniciar)\s+', '', raw, flags=re.I).strip()
                q = re.sub(r'^(o|a|um|uma)\s+', '', q, flags=re.I).strip()
