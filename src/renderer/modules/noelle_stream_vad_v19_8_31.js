...
/*
  ...
  Detecta fala/silêncio usando o nível de áudio emitido pelo módulo de microfone V19.8.30.
  Não grava áudio.
  Não transcreve.
  Não chama IA.
  Não gera voz.
...
(() => {
  ...
  function ensurePanel() {
    ...
    card.setAttribute("data-noelle-vad-v19-8-31", "true");
    card.innerHTML = `
      <h3>VAD simples</h3>
      <div class="stream-v19831-vad-grid">
        <div>
          <strong id="streamVadStateV19831">Aguardando microfone</strong>
          <p id="streamVadDetailV19831" class="muted">A Fase 3 detecta fala e silêncio. Ainda não existe transcrição.</p>
        </div>
        <div class="stream-v19831-vad-stats">
          <span>Segmentos: <b id="streamVadSegmentsV19831">0</b></span>
          <span>Último trecho: <b id="streamVadLastSegmentV19831">0ms</b></span>
        </div>
      </div>
    `;
  ...
  function updatePanel(label, detail) {
    ...
    const stateEl = qs("#streamVadStateV19831");
    const detailEl = qs("#streamVadDetailV19831");
    const segmentsEl = qs("#streamVadSegmentsV19831");
    const lastSegmentEl = qs("#streamVadLastSegmentV19831");
  ...
  function startSpeech(ts) {
    state.speaking = true;
    state.speechStartedAt = ts;
    state.lastLoudAt = ts;
    state.segments += 1;

    setStatus("Fala detectada", "VAD detectou que você começou a falar. Ainda não há transcrição.", "ready_to_answer");
    updatePanel("Fala detectada", "Continue falando. O trecho fecha depois de quase 1 segundo de silêncio.");
    addLog("vad", "Fala detectada.");
    emit("noelle-stream-vad-start-v19831", { level: state.level, at: ts });
  }
  ...
  function resetVad(reason = "VAD parado.") {
    ...
    if (wasSpeaking) {
      addLog("vad", "Fala cancelada porque o microfone parou.");
      emit("noelle-stream-vad-cancel-v19831", { reason });
    }
  }
  ...
  function onAudioLevel(event) {
    ...
    if (!state.speaking) {
      if (level >= config.speechThreshold) {
        ...
        if (candidateMs >= config.minSpeechMs) {
          startSpeech(ts);
        }
      } else {
        state.candidateSpeechAt = 0;
    ...
    if (level >= config.releaseThreshold) {
      state.lastLoudAt = ts;
      updatePanel("Falando", `Volume atual: ${Math.round(level * 100)}%.`);
      return;
    }
    ...
    if (quietMs >= config.silenceToFinishMs) {
      finishSpeech(ts);
    }
  }

  function bind() {
    if (window.__NOELLE_STREAM_VAD_V19831_BOUND__) return;
    window.__NOELLE_STREAM_VAD_V19831_BOUND__ = true;
    ...
    document.addEventListener("click", (event) => {
      ...
      if (target.id === "streamStopBtn") {
        setTimeout(() => resetVad("Microfone desligado pelo botão."), 120);
      }
  ...
  window.NoelleStreamVadV19831 = Object.freeze({
    version: VERSION,
    bind,
    resetVad,
    getState
