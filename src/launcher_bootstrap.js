...
async function importAvatar() {
  ...
  try {
    const result =
      await window.desktopWidget?.launcherImportAvatar?.() ||
      await window.desktopWidget?.importAvatarVrmNative?.();
...
async function main() {
  ...
  bind("settingsBtn", async () => {
    setStatus("Abrindo configurações...");
    try {
      await window.desktopWidget?.launcherOpenConfig?.();
      setStatus("Configurações abertas.");
    } catch (err) {
      setStatus("Falha ao abrir configurações.");
      console.error(err);
    }
  });
